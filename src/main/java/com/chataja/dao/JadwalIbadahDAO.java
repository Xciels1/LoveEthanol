...
public class JadwalIbadahDAO {
    ...
    /**
     * Ambil jadwal ibadah untuk minggu depan
     * (dari Senin depan sampai Minggu depan).
     */
    public List<JadwalIbadah> getNextWeek() {
        List<JadwalIbadah> list = new ArrayList<>();
        LocalDate today = LocalDate.now();

        // Hitung Senin minggu depan
        java.time.DayOfWeek todayDow = today.getDayOfWeek();
        int daysUntilNextMonday = (java.time.DayOfWeek.MONDAY.getValue() - todayDow.getValue() + 7) % 7;
        if (daysUntilNextMonday == 0) daysUntilNextMonday = 7; // jika hari ini Senin → Senin depan
        ...
        try (Connection conn = DatabaseManager.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, nextMonday.toString());
            ps.setString(2, nextSunday.toString());
            ResultSet rs = ps.executeQuery();
            while (rs.next()) list.add(map(rs));
        } catch (SQLException e) {
            System.err.println("[JadwalIbadahDAO] getNextWeek error: " + e.getMessage());
        }
        return list;
    }
